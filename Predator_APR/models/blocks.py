...
def block_decider(block_name,
    ...
    if block_name == 'last_unary':
        if config.switch_to_decoder and config.symmetric:
            return LastUnaryBlock(in_dim, config.point_generation_ratio * 3, config.use_batch_norm, config.batch_norm_momentum)
        else:
