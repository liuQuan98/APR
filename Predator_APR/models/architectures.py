...
class KPFCNNDecoder(nn.Module):

    def __init__(self, config):
        super(KPFCNNDecoder, self).__init__()
        ...
        layer = 0
        r = config.first_subsampling_dl * config.conv_radius
        in_dim = config.final_feats_dim
        out_dim = config.first_feats_dim
        # This parameter does not actually change model output dims. it's hard-coded at models/blocks.py in LastUnary block.
        # self.final_feats_dim = config.point_generation_ratio * 3 
    ...
    def forward(self, batch):
        # Get input features
        x = batch['second_features'].clone().detach()
