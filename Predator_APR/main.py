...
from models.architectures import KPFCNN, KPFCNNDecoder
from models.mlp import get_GenerativeMLP
...
if __name__ == '__main__':
    # indicate the gpu this thread is using
    print(f"Using GPU number " + os.environ["CUDA_VISIBLE_DEVICES"])
    # load configs
    ...
    config.switch_to_decoder = False
    config.architecture = architectures[config.dataset]
    config.model = KPFCNN(config)
    config.model_name = "KPFCNN"

    # build symmetrical or asymmetrical decoders
    config.switch_to_decoder = True
    if config.symmetric:
        config.generative_model = KPFCNNDecoder(config)
    else:
        config.generative_model = get_GenerativeMLP(config)
    ...
    if config.optimizer == 'SGD':
        config.optimizer = optim.SGD(
            params = [
                ...
                {'params': config.generative_model.parameters()}
            ],
            lr=config.lr,
        ...
        config.optimizer = optim.Adam(
            params = [
                {'params': config.model.parameters()},
                {'params': config.generator_model.parameters()}
            ],
            lr=config.lr,
            betas=(0.9, 0.999),
            weight_decay=config.weight_decay,
