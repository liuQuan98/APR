...
# class GenerateChamferLoss(nn.Module):

#     def __init__(self, config):
#         super(GenerateChamferLoss,self).__init__()
#         self.device = config.device
#         self.loss_ratio = config.inner_loss_ratio
#         self.point_generation_ratio = config.point_generation_ratio
#         self.regularization_strength = config.inner_regularization_strength
...
#         # print(f"len1: {n1}, len2: {n2}, forward: {forward_cd_dist}, backward: {backward_cd_dist}")
#         return forward_cd_dist / n1 + backward_cd_dist / n2
    
#     def forward(self, radius, gen_feats, src_pcd, tgt_pcd, regularization_strength=None):
#         generative_loss = 0 # weighted sum of chamfer loss and regularization loss
#         if regularization_strength is None:
#             regularization_strength = self.regularization_strength

#         # note that regularization loss is divided by radius^2
#         # in order to accomodate to different convolution radius at different layers
#         regularize_loss = torch.mean(torch.sum((gen_feats.reshape(-1, 3) / radius)**2, axis=-1))
#         mod_gen_feats = (gen_feats + src_pcd.repeat(1, self.point_generation_ratio)).reshape(-1, 3)
#         chamfer_loss_raw = self.chamfer_distance(mod_gen_feats, 
#                                                  tgt_pcd.to(self.device))
#         generative_loss = (chamfer_loss_raw + \
#                            regularize_loss * self.regularization_strength) * self.loss_ratio
#         return generative_loss, chamfer_loss_raw, regularize_loss
