...
class KITTIPairDataset(PairDataset):
  ...
  def __getitem__(self, idx):
    ...
    if len(matches) < 1000:
      ...
      if len(matches) == 0:
        print("length = 0!")
      print(f"Matching indices small at {drive}, {t0}, {t1},len()={len(matches)}")
...
class KITTIRandDistPairDataset(KITTIPairDataset):
  r"""
  Generate KITTI pairs with random distance bwtween frames
  """
  MIN_DIST = 5
  MAX_DIST = 45
  ...
  def __init__(self,
    ...
    if self.IS_ODOMETRY:
      for dirname in subset_names:
        ...
        curr_time = inames[0]
        while curr_time in inames:
          # calculate the distance by random
          dist_tmp = self.MIN_DIST + np.random.rand() * (self.MAX_DIST - self.MIN_DIST)
          # Find the min index
          next_time = np.where(pdist[curr_time][curr_time:curr_time + int(dist_tmp*10)] > dist_tmp)[0]
          if len(next_time) == 0:
            curr_time += 1
          else:
            # Follow https://github.com/yewzijian/3DFeatNet/blob/master/scripts_data_processing/kitti/process_kitti_data.m#L44
...
ALL_DATASETS = [ThreeDMatchPairDataset, KITTIPairDataset, KITTINMPairDataset, KITTIRandDistPairDataset]
